...
- **낮 시간대 30분마다 상태 교정** (`run_sync.py`)
  - 오늘 카드의 `진행 상태`를 **체크박스 진실에 맞춰 강제 동기화**
  - 전부 체크 → `인증 완료`, 아니면 `계획 완료`(수동으로 바꾼 가짜 인증완료/실패를 되돌림)
  - Notion은 특정 속성만 잠글 수 없어, 상태를 봇이 계속 덮어써 사실상 봇 소유로 만든다
